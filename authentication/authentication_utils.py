...
from django.contrib.auth.hashers import make_password, check_password as _check_hash
...
def generate_setupkey(length=10):
    """Generate a plain-text 10-character alphanumeric setup key."""
    chars = string.ascii_letters + string.digits
    return "".join(random.choices(chars, k=length))


def hash_setupkey(plain_key):
    """Return a Django-hashed version of the setup key."""
    return make_password(plain_key)


def verify_setupkey(plain_key, hashed_key):
    """Return True if the plain key matches the stored hash."""
    if not plain_key or not hashed_key:
        return False
    return _check_hash(plain_key, hashed_key)


def make_unique_username(base, existing_qs):
    """
    Given a base string and a queryset (or callable returning bool),
    return a unique username by appending a digit if needed.
    existing_qs should be a callable: lambda u: CustomUser.objects.filter(...).exists()
    """
    candidate = re.sub(r"[^\w.@+-]", "", base.lower())[:140]
    if not existing_qs(candidate):
        return candidate
    counter = 1
    while True:
        new_candidate = f"{candidate}{counter}"
        if not existing_qs(new_candidate):
...
def _login_required(request):
    """Returns True if user is authenticated, otherwise adds error message."""
...
def _staff_required(request):
    """Returns True if user is authenticated and has staff/admin/sys_admin role."""
    if not request.user.is_authenticated:
        messages.error(request, "Please log in to access this page.")
        return False
    allowed = ("staff", "admin", "sys_admin")
...
def send_staff_setup_email(email, first_name, username, plain_setupkey, login_url):
    """
    Email sent to a newly created staff account containing their setup credentials.
    """
    _send_mail_safe(
        "Your Staff Account – Setup Instructions",
        f"Hello {first_name},\n\n"
        f"A staff account has been created for you on our platform.\n"
        f"Your account is not yet active. Please follow the steps below to complete your setup.\n\n"
        f"─────────────────────────────────────\n"
        f"  Your Credentials\n"
        f"─────────────────────────────────────\n"
        f"  Username  : {username}\n"
        f"  Setup Key : {plain_setupkey}\n"
        f"─────────────────────────────────────\n\n"
        f"Setup Steps:\n"
        f"  1. Visit the login page: {login_url}\n"
        f"  2. Click the \"Proceed with Setup Key\" button.\n"
        f"  3. Enter your Username and Setup Key.\n"
        f"  4. You will receive a verification OTP on this email — enter it.\n"
        f"  5. Set your new permanent password.\n\n"
        f"Keep these credentials safe and do not share them.\n"
        f"Once you complete setup, your Setup Key will be invalidated.\n\n"
        f"Regards,\nThe Team",
        email,
    )


def send_staff_account_live_email(email, first_name):
    """Notify staff that their account is now active."""
    _send_mail_safe(
        "Your Account is Now Active",
        f"Hello {first_name},\n\n"
        f"Your account has been successfully activated and your password has been set.\n"
        f"You can now log in using your username and the new password you just created.\n\n"
        f"Regards,\nThe Team",
        email,
    )
