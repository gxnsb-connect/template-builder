...
def user_login(request):
    ...
    if "@" in username_or_email:
        try:
            found = CustomUser.objects.get(email__iexact=username_or_email)
...
def fp_verify_identity(request):
    ...
    if mode == "email":
        ...
        if entered_email.lower() != real_email.lower():
            ...
            if email_tries >= 2:
                request.session["fp_mode"] = "username"
                messages.warning(request, "Email does not match. You have used all email attempts. Please verify using your username.")
            else:
                messages.error(request, f"Email does not match. You have {2 - email_tries} attempt(s) remaining.")
            return redirect(reverse("fp_verify_identity"))
    ...
    if mode == "username":
        ...
        if entered_username.lower() != real_username.lower():
            ...
            messages.error(request, f"Username does not match. You have {2 - user_tries} attempt(s) remaining.")
            return redirect(reverse("fp_verify_identity"))
